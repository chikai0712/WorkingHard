...
GLOBALPING_TOKEN="${GLOBALPING_TOKEN:-uh5vlg4ttg3v5gwby5zgtqrciimahql5}"
...
# 載入 Telegram 通知函數（如果存在）
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [ -f "$SCRIPT_DIR/telegram-notify.sh" ]; then
    source "$SCRIPT_DIR/telegram-notify.sh"
    load_telegram_config "$SCRIPT_DIR/telegram-config.env" 2>/dev/null || true
...
process() {
    ...
    if [ $? -ne 0 ]; then
        ...
        if [ "$TELEGRAM_AVAILABLE" = true ]; then
            send_telegram_domain_result "$domain" "API_ERROR" "API 請求失敗"
        fi
    ...
    # Telegram 通知（根據配置）
    if [ "$TELEGRAM_AVAILABLE" = true ]; then
        send_telegram_domain_result "$domain" "$overall" "$(echo -e "$details")"
    fi
}
...
main() {
    ...
    echo "========================================"
    echo "域名檢測腳本 v3.3 - Telegram 通知版"
    echo "========================================"
    echo "域名文件: $DOMAINS_FILE"
    echo "總域名數: $TOTAL"
    echo "Telegram 通知: $([ "$TELEGRAM_AVAILABLE" = true ] && echo "啟用" || echo "未配置")"
    echo "========================================"
    echo ""
    ...
    # Telegram 摘要通知
    if [ "$TELEGRAM_AVAILABLE" = true ]; then
        send_telegram_summary "$TOTAL" "$CLEAN" "$BLOCKED" "$TIMEOUT" "$WARNING" "$PARTIAL" "$API_ERROR"
