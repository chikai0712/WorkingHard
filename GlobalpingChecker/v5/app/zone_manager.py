"""
GlobalpingChecker V5 - Zone Manager
...
class ZoneManager:
    ...
    def reset_normal_to_pending(self) -> int:
        """
        AM 00:01 全量重置：將正常區所有域名移回 PENDING，
        讓下一輪 ABNORMAL_CHECK 循環把它們全部重新檢測一遍。
        回傳重置的域名數量。
        """
        updated = (
            self.db.query(Domain)
            .filter(Domain.zone == DomainZone.NORMAL)
            .update(
                {
                    "zone": DomainZone.PENDING,
