"""
GlobalpingChecker V5 - Node Pool Manager
管理和驗證節點池
...
4. 依 TOP30 ISP 優先序排列，其餘節點附後
5. 存入 node_pool 資料表供 checker.py 取用

V5 變更：
- is_top_isp 欄位（rank <= 30 時自動設為 True）
- 使用 v5 models.NodePool
"""
import asyncio
import httpx
...
class NodePoolManager:
    ...
    def get_country_nodes(self, country: str, limit: int = 3) -> List[str]:
        ...
        優先回傳 TOP30 ISP 節點。
        注意：回傳的 node_id 為內部 hash ID，不可直接用於 Globalping locations。
        請改用 get_country_nodes_for_measurement()。
        """
        db = SessionLocal()
        try:
            nodes = (
    ...
    def get_country_nodes_for_measurement(self, country: str, limit: int = 5) -> List[Dict]:
        """
        從資料庫取出指定國家的 TOP ISP 節點作為優先 locations，
        並在末尾補一個純 country fallback，確保不足時 Globalping 自動補充在線節點。
        格式：[{"country": "ID", "city": "Jakarta", "asn": 12345}, ..., {"country": "ID"}]
        """
        db = SessionLocal()
        try:
            nodes = (
            ...
            def _make_loc(n):
                loc: Dict = {"country": n.country}
                if n.city and n.city != "Unknown":
                    loc["city"] = n.city
                try:
                    asn_val = int(n.asn)
                    if asn_val:
            ...
            # Pass 1：每個不同 isp_rank 各取一個（TOP 越小越優先）
            for n in nodes:
                if len(locations) >= limit:
                    break
                if n.isp_rank in seen_ranks:
                    continue
                seen_ranks.add(n.isp_rank)
                seen_cities.add(n.city)
                locations.append(_make_loc(n))

            # Pass 2：補充不同城市節點（地理分散）
            if len(locations) < limit:
                for n in nodes:
                    if len(locations) >= limit:
                        break
                    if n.city in seen_cities:
                        continue
                    seen_cities.add(n.city)
                    locations.append(_make_loc(n))

            # Fallback：在 locations 末尾加一個純 country entry
            # 讓 Globalping 在指定節點都不在線時自動從該國在線節點補充
            country_fallback = {"country": country}
            if country_fallback not in locations:
                locations.append(country_fallback)
    ...
    def get_isp_rank(self, isp_name: str) -> int:
        """取得 ISP 排名（供 checker.py 計算 current_node_top 使用）"""
        return _isp_priority(isp_name)
