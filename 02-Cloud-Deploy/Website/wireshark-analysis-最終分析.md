...
|------|------|-----------|---------|------|
| 38.339s | 查詢 | 205.251.197.44 (AWS) | 38.379s (40ms) | ✅ 成功 |
| 38.398s | 查詢 | 8.8.8.8 (Google) | 38.428s (30ms) | ✅ 成功 |
| 54.928s | 查詢 | 205.251.197.44 (AWS) | 54.970s (42ms) | ✅ 成功 |
...
1. **所有 DNS 查詢都成功**
   - AWS DNS (205.251.197.44)：40ms 和 42ms 回應
   - Google DNS (8.8.8.8)：30ms 回應

2. **查詢時間間隔**
   - 第一次：38.339s (AWS) 和 38.398s (Google) = **59ms 間隔**
   - 第二次：54.928s (AWS) = **16.5 秒後**
...
證據：
1. AWS DNS (205.251.197.44) 成功回應（40ms）
2. 沒有 ICMP 錯誤
3. 查詢時間差只有 59ms（並行查詢）
...
```
38.339s → 查詢 AWS DNS (205.251.197.44)
38.379s ← AWS DNS 成功回應（40ms，沒有 ICMP 錯誤）
38.398s → 查詢 Google DNS (8.8.8.8) [只差 59ms，並行查詢]
38.428s ← Google DNS 成功回應（30ms）
```
