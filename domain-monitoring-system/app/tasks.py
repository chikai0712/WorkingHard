...
def check_single_domain(domain_id: int):
    ...
    try:
        ...
        logger.info(f"Checked domain {domain.domain}: {status}")
        return {"domain": domain.domain, "status": status}
...
def analyze_and_alert(domain_id: int, dns_result: dict):
    ...
    try:
        ...
        checks = {
            'global_dns': {
                'status': 'ok' if dns_result.get('success_rate', 0) > 0.5 else 'fail',
                'resolved_ips': []
            },
            'isp_dns': {
                'failed_isps': dns_result.get('failed_nameservers', []),
        ...
        if alert_data:
            ...
            if existing_alert:
                ...
                logger.info(f"Created and sent alert for {domain.domain}")
