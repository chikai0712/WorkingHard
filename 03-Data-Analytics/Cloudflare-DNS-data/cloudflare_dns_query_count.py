...
# Force bypass all proxy settings (system proxy may interfere)
for _var in ["HTTP_PROXY", "HTTPS_PROXY", "http_proxy", "https_proxy", "ALL_PROXY", "all_proxy"]:
    os.environ.pop(_var, None)

# Patch requests Session to disable trust_env globally
...
def get_zones(headers: Dict, account_id: Optional[str], filter_names: List[str]) -> List[Dict]:
    ...
    while True:
        ...
        def _call():
            resp = requests.get(f"{API_BASE}/zones", headers=headers, params=params, timeout=30, proxies={"http": None, "https": None})
