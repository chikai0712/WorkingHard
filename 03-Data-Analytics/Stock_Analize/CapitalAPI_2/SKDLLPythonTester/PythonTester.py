...
def on_GetOpenInterestGW_click():
    app = MainApp.instance
    order_ui = app.order_frame
    login_ui = app.login_frame
    ...
    if order_ui.boxGetOpenInterestGWFormat.current() < 0:
        messagebox.showinfo("錯誤", "請選擇格式")
        return
    nFormat = order_ui.boxGetOpenInterestGWFormat.current()

    result = SK.GetOpenInterestGW(loginID, account, nFormat)

    # 顯示回傳訊息
    login_ui.after(0, lambda: login_ui.listOnReplyMessage.insert(
        "end", f"[GetOpenInterestGW]回傳值: {result.StatusCode} {result.Message}"
    ))
    ...
    if result.StatusCode == 0 and len(result.Blocks) > 0:
        for block in result.Blocks:
            ...
            login_ui.after(0, lambda b=block: login_ui.listOnReplyMessage.insert(
                "end",
                f"帳號資料: {b.MarketType.decode('ansi')} "
                f"-({b.Account.decode('ansi')}-{b.LoginID.decode('ansi')})"
            ))

            # 第二行：商品、部位與費用資訊
            login_ui.after(0, lambda b=block: login_ui.listOnReplyMessage.insert(
                "end",
                f"商品: {b.FutureNo.decode('ansi')}, "
                f"買賣別: {b.BuySell.decode('ansi')}, "
                f"未平倉口數: {b.Qty.decode('ansi')}, "
                f"當沖未平口數: {b.Qty_Trade.decode('ansi')}, "
                f"平均成本: {b.AvgPrice.decode('ansi')}, "
                f"單口手續費: {b.Fee.decode('ansi')}, "
                f"交易稅: {b.Tax.decode('ansi')}"
            ))
        ...
        login_ui.after(0, lambda: login_ui.listOnReplyMessage.insert(
            "end", "[GetOpenInterestGW]沒有資料或回傳錯誤"
        ))  
        
def on_GetOFOpenInterestGW_click():
    app = MainApp.instance
    order_ui = app.order_frame
    login_ui = app.login_frame
    ...
    if order_ui.boxGetOFOpenInterestGW.current() < 0:
        messagebox.showinfo("錯誤", "請選擇格式")
        return
    nFormat = order_ui.boxGetOFOpenInterestGW.current()
    
    result = SK.GetOFOpenInterestGW(loginID, account, nFormat)

    # 顯示回傳訊息
    login_ui.after(0, lambda: login_ui.listOnReplyMessage.insert(
        "end", f"[GetOFOpenInterestGW]回傳值: {result.StatusCode} {result.Message}"
    ))
    ...
    if result.StatusCode == 0 and len(result.Blocks) > 0:
        for block in result.Blocks:
            ...
            login_ui.after(0, lambda b=block: login_ui.listOnReplyMessage.insert(
                "end",
                f"帳號資料: {b.IsOption.decode('ansi')}-({b.Account.decode('ansi')}-{b.LoginID.decode('ansi')})"
            ))

            # 期貨與選擇權分支
            if block.IsOption.decode('ansi') == "OF":
                login_ui.after(0, lambda b=block: login_ui.listOnReplyMessage.insert(
                    "end",
                    f"交易所: {b.ExchangeName.decode('ansi')} {b.ExchangeCode.decode('ansi')}, "
                    f"商品: {b.ProductNameWithYM.decode('ansi')} {b.ProductCodeWithYM.decode('ansi')}, "
                    f"買賣別: {b.BuySell.decode('ansi')}, 未平倉口數: {b.OpenInterest.decode('ansi')}, "
                    f"平均價: {b.AvgPrice.decode('ansi')}, 現價: {b.MarketPrice.decode('ansi')}, "
                    f"未平損益: {b.UnrealizedPL.decode('ansi')}, "
                    f"委託買價/量: {b.BidPrice.decode('ansi')}/{b.BidVolume.decode('ansi')}, "
                    f"委託賣價/量: {b.AskPrice.decode('ansi')}/{b.AskVolume.decode('ansi')}, "
                    f"賣方新倉口數: {b.AskNewOpenVolume.decode('ansi')}, "
                    f"昨日結算價: {b.PrevSettlementPrice.decode('ansi')}, "
                    f"當沖未平口數: {b.DayTradeOpenInterest.decode('ansi')}, "
                    f"原始保證金: {b.InitialMargin.decode('ansi')}"
                ))
            elif block.IsOption.decode('ansi') == "OO":
                login_ui.after(0, lambda b=block: login_ui.listOnReplyMessage.insert(
                    "end",
                    f"交易所: {b.ExchangeName.decode('ansi')} {b.ExchangeCode.decode('ansi')}, "
                    f"商品: {b.ProductNameWithYM.decode('ansi')} {b.ProductCodeWithYM.decode('ansi')}, "
                    ...
                    f"當沖未平口數: {b.DayTradeOpenInterest.decode('ansi')}, "
                    f"原始保證金: {b.InitialMargin.decode('ansi')}, "
                    f"履約價: {b.StrikePrice.decode('ansi')}, C/P: {b.CallPut.decode('ansi')}"
                ))
        ...
        login_ui.after(0, lambda: login_ui.listOnReplyMessage.insert(
            "end", "[GetOFOpenInterestGW]沒有資料或回傳錯誤"
        )) 

def on_GetRealBalanceReport_click():
    app = MainApp.instance
    order_ui = app.order_frame
    login_ui = app.login_frame
    ...
    result = SK.GetRealBalanceReport(loginID, account)

    # 顯示回傳訊息
    login_ui.after(0, lambda: login_ui.listOnReplyMessage.insert(
        "end", f"[GetRealBalanceReport]回傳值: {result.StatusCode} {result.Message}"
    ))
    ...
    if result.StatusCode == 0 and len(result.Blocks) > 0:
        for block in result.Blocks:
            ...
            login_ui.after(0, lambda b=block: login_ui.listOnReplyMessage.insert(
                "end",
                f"帳號資料: {b.LoginID.decode('ansi')}-{b.AccountNo.decode('ansi')}"
            ))

            login_ui.after(0, lambda b=block: login_ui.listOnReplyMessage.insert(
                "end",
                f"股票代號: {b.StockCode.decode('ansi')}, "
                f"庫存種類: {b.InventoryType.decode('ansi')}, "
                f"資額度(原始): {b.CreditLimitOriginal.decode('ansi')}, "
                f"資額度(可用): {b.CreditLimitAvailable.decode('ansi')}, "
                f"券額度(原始): {b.ShortLimitOriginal.decode('ansi')}, "
                f"券額度(可用): {b.ShortLimitAvailable.decode('ansi')}, "
                f"昨日庫存: {b.YesterdayInventory.decode('ansi')}, "
                f"今日委買: {b.TodayBidQty.decode('ansi')}, "
                f"今日委賣: {b.TodayAskQty.decode('ansi')}, "
                f"今日買進成交: {b.TodayBuyMatched.decode('ansi')}, "
                f"今日賣出成交: {b.TodaySellMatched.decode('ansi')}, "
                f"今日資券可賣出/回補: {b.AvailableToSellOrCover.decode('ansi')}, "
                f"可資沖股數: {b.DayTradeBuyableQty.decode('ansi')}, "
                f"可券沖股數: {b.DayTradeSellableQty.decode('ansi')}, "
                f"即時庫存: {b.RealTimeInventory.decode('ansi')}, "
                f"即時個股維持率: {b.RealTimeMarginRate.decode('ansi')}"
            ))
        ...
        login_ui.after(0, lambda: login_ui.listOnReplyMessage.insert(
            "end", "[GetRealBalanceReport]沒有資料或回傳錯誤"
        )) 

def on_GetFutureRights_click():
    app = MainApp.instance
    order_ui = app.order_frame
    login_ui = app.login_frame
    ...
    if order_ui.boxGetFutureRights_CoinType.current() < 0:
        messagebox.showinfo("錯誤", "請選擇幣別")
        return
    nCoinType = order_ui.boxGetFutureRights_CoinType.current()
    
    result = SK.GetFutureRights(loginID, account, nCoinType)

    # 顯示回傳訊息
    login_ui.after(0, lambda: login_ui.listOnReplyMessage.insert(
        "end", f"[GetFutureRights]回傳值: {result.StatusCode} {result.Message}"
    ))
    ...
    if result.StatusCode == 0 and len(result.Blocks) > 0:
        for block in result.Blocks:
            ...
            login_ui.after(0, lambda b=block: login_ui.listOnReplyMessage.insert(
                "end",
                f"帳號資料:({b.AccountNo.decode('ansi')}-{b.LoginID.decode('ansi')})"
            ))

            login_ui.after(0, lambda b=block: login_ui.listOnReplyMessage.insert(
                "end",
                f"帳戶餘額: {b.AccountBalance.decode('ansi')}, "
                f"浮動損益: {b.FloatingPL.decode('ansi')}, "
                f"已實現費用: {b.RealizedFee.decode('ansi')}, "
                f"交易稅: {b.TransactionTax.decode('ansi')}, "
                f"預扣權利金: {b.WithheldPremium.decode('ansi')}, "
                f"權利金收付: {b.PremiumPayment.decode('ansi')}, "
                f"權益數: {b.Equity.decode('ansi')}, "
                f"超額保證金: {b.ExcessMargin.decode('ansi')}, "
                f"存提款: {b.DepositWithdrawal.decode('ansi')}, "
                f"買方市值: {b.BuyerMarketValue.decode('ansi')}, "
                f"賣方市值: {b.SellerMarketValue.decode('ansi')}, "
                f"期貨平倉損益: {b.FuturesClosePL.decode('ansi')}, "
                f"盤中未實現: {b.IntradayUnrealized.decode('ansi')}, "
                f"原始保證金: {b.InitialMargin.decode('ansi')}, "
                f"維持保證金: {b.MaintenanceMargin.decode('ansi')}, "
                f"部位原始保證金: {b.PositionInitialMargin.decode('ansi')}, "
                f"部位維持保證金: {b.PositionMaintenanceMargin.decode('ansi')}, "
                f"委託保證金: {b.OrderMargin.decode('ansi')}, "
                f"超額最佳保證金: {b.ExcessOptimalMargin.decode('ansi')}, "
                f"權利總值: {b.TotalPremiumValue.decode('ansi')}, "
                f"預扣費用: {b.WithheldFee.decode('ansi')}, "
                f"原始保證金(原始): {b.OriginalInitialMargin.decode('ansi')}, "
                f"昨日餘額: {b.YesterdayBalance.decode('ansi')}, "
                f"選擇權組合單加不加收保證金: {b.OptionComboMarginFlag.decode('ansi')}, "
                f"維持率: {b.MaintenanceRate.decode('ansi')}, "
                f"幣別: {b.Currency.decode('ansi')}, "
                f"足額原始保證金: {b.FullInitialMargin.decode('ansi')}, "
                f"足額維持保證金: {b.FullMaintenanceMargin.decode('ansi')}, "
                f"足額可用: {b.FullAvailable.decode('ansi')}, "
                f"抵繳金額: {b.OffsetAmount.decode('ansi')}, "
                f"有價可用: {b.SecuritiesAvailable.decode('ansi')}, "
                f"可用餘額: {b.AvailableBalance.decode('ansi')}, "
                f"足額現金可用: {b.FullCashAvailable.decode('ansi')}, "
                f"有價價值: {b.SecuritiesValue.decode('ansi')}, "
                f"風險指標: {b.RiskIndicator.decode('ansi')}, "
                f"選擇權到期差異: {b.OptionExpiryDifference.decode('ansi')}, "
                f"選擇權到期差損: {b.OptionExpiryPL.decode('ansi')}, "
                f"期貨到期損益: {b.FuturesExpiryPL.decode('ansi')}, "
                f"加收保證金: {b.AdditionalMargin.decode('ansi')}"
            ))
        ...
        login_ui.after(0, lambda: login_ui.listOnReplyMessage.insert(
            "end", "[GetFutureRights]沒有資料或回傳錯誤"
        )) 

def on_GetOFFutureRights_click():
    app = MainApp.instance
    order_ui = app.order_frame
    login_ui = app.login_frame
    ...
    if order_ui.boxTypeGetOFFutureRights_CoinType.current() < 0:
        messagebox.showinfo("錯誤", "請選擇幣別")
        return
    nCoinType = order_ui.boxTypeGetOFFutureRights_CoinType.current()
    
    result = SK.GetOFFutureRights(loginID, account, nCoinType)

    # 顯示回傳訊息
    login_ui.after(0, lambda: login_ui.listOnReplyMessage.insert(
        "end", f"[GetOFFutureRights]回傳值: {result.StatusCode} {result.Message}"
    ))
    ...
    if result.StatusCode == 0 and len(result.Blocks) > 0:
        for block in result.Blocks:
            ...
            login_ui.after(0, lambda b=block: login_ui.listOnReplyMessage.insert(
                ...
                f"風險指標: {b.RiskIndicator.decode('ansi')}, "
                f"足額維持保證金: {b.FullMaintenanceMargin.decode('ansi')}, "
                f"可動用保證金: {b.WithdrawableMargin.decode('ansi')}, "
                f"委託保證金: {b.OrderMargin.decode('ansi')}, "
                f"維持保證金: {b.MaintenanceMargin.decode('ansi')}, "
                f"原始保證金: {b.InitialMargin.decode('ansi')}, "
                f"淨值權益數: {b.NetEquity.decode('ansi')}, "
                f"預扣權利金: {b.WithheldPremium.decode('ansi')}, "
                f"未沖銷買方選擇權市值: {b.OptionLongMarketValue.decode('ansi')}, "
                f"未沖銷賣方選擇權市值: {b.OptionShortMarketValue.decode('ansi')}, "
                f"帳戶權益: {b.AccountEquity.decode('ansi')}, "
                f"盤中未實現: {b.IntradayUnrealized.decode('ansi')}, "
                f"未沖銷期貨浮動損益: {b.FuturesFloatingPL.decode('ansi')}, "
                f"今日權利金支付: {b.PremiumPaidToday.decode('ansi')}, "
                f"本日期貨平倉損益淨額: {b.FuturesClosePLToday.decode('ansi')}, "
                f"已實現費用: {b.RealizedFee.decode('ansi')}, "
                f"今日存提款: {b.DepositWithdrawalToday.decode('ansi')}, "
                f"前日帳戶餘額: {b.PreviousDayBalance.decode('ansi')}, "
                f"參考匯率: {b.ReferenceRate.decode('ansi')}, "
                f"選擇權到期差益: {b.OptionExpiryGain.decode('ansi')}, "
                f"選擇權到期差損: {b.OptionExpiryLoss.decode('ansi')}, "
                f"期貨交割損益: {b.FuturesDeliveryPL.decode('ansi')}, "
                f"超額/追繳保證金: {b.ExcessOrMarginCall.decode('ansi')}, "
                f"到期履約損益: {b.ExerciseSettlementPL.decode('ansi')}, "
                f"帳戶餘額: {b.AccountBalance.decode('ansi')}, "
                f"固定保證金: {b.FixedMargin.decode('ansi')}, "
                f"變動保證金: {b.VariableMargin.decode('ansi')}"
            ))
        ...
        login_ui.after(0, lambda: login_ui.listOnReplyMessage.insert(
            "end", "[GetOFFutureRights]沒有資料或回傳錯誤"
        )) 
...
if __name__ == "__main__":
    ...
    app.order_frame.btnGetOpenInterestGW.config(command=on_GetOpenInterestGW_click)
    app.order_frame.btnGetOFOpenInterestGW.config(command=on_GetOFOpenInterestGW_click)
    app.order_frame.btnGetRealBalanceReport.config(command=on_GetRealBalanceReport_click)
    app.order_frame.btnGetFutureRights.config(command=on_GetFutureRights_click)
    app.order_frame.btnGetOFFutureRights.config(command=on_GetOFFutureRights_click)
